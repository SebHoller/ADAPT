...
public class StlAssetPostProcessor : AssetPostprocessor
{
    ...
    private static void createStlPrefab(string stlFile)
    {
        ...
        PrefabUtility.SaveAsPrefabAssetAndConnect(gameObject, getPrefabAssetPath(stlFile), InteractionMode.AutomatedAction);
        Object.DestroyImmediate(gameObject);
    }

    // changed from private to public for this project
    public static GameObject CreateStlParent(string stlFile)
    {
        Mesh[] meshes = Importer.Import(stlFile);
        if (meshes == null)
            return null;
